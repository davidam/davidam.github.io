...
* Test simples, medias, medianas, proporciones
#+BEGIN_SRC R
library(stats)
summary(b)
#+END_SRC

** Comparamos si nuestra variable "Y" es igual a un valor ya publicado

Wilcoxon Rank-Sum Test para evaluar si la mediana de mi muestra es
igual o no a otra ya publicada.n. "mu", es el valor de la bibliografía

Simple t-test para evaluar si la media de mi muestra es igual o no a otra
ya publicada.

#+BEGIN_SRC R
wilcox.test(Peso, mu=65)
summary(Peso)
t.test(Peso, mu=55)
#+END_SRC

t-test para evaluar si hay diferencias entre las medias en dos
muestras independientes, p.ej. entre dos poblaciones o entre dos
estaciones del año (si los animales son cazados y sólo se pesan una
vez)

Podemos explorar cuestiones de este tipo haciendo gráficos boxplot

#+BEGIN_SRC R
library(gplots)
plotmeans(Peso~Manejo, ylab="Peso en Kg", xlab="Manejo")
#+END_SRC

También podemos explorar los valores numericos de la variable peso en
cada grupo.

#+BEGIN_SRC R
library(plotrix)

me=  tapply (Peso,  Manejo,mean)
med= tapply(Peso,  Manejo,median)
Sd = tapply(Peso, Manejo, sd)
Se = tapply (Peso,  Manejo,std.error)
Mi = tapply (Peso,  Manejo,min)
Ma = tapply (Peso,  Manejo,max)
size= tapply (Peso,  Manejo,length)

cbind(Mean = me, Median =med,SD= Sd, SE = Se, Min = Mi, Max = Ma, n = size)
t(cbind(mean = me, SD = Sd,SE=Se, Min = Mi, Max = Ma, n = size))
#+END_SRC

** Mann-Whitney test para comparar si hay diferencias entre medianas muestras independientes
